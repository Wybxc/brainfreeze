...
let indent_string str =
  ...
  let indented = List.map (fun line -> if line = "" then line else "  " ^ line) lines in
  String.concat "\n" indented
...
let string_of_literal = function
  | LByte b -> Printf.sprintf "0x%02X" b
  | LByteArray bytes ->
      let byte_strs = List.map (fun b -> Printf.sprintf "0x%02X" b) bytes in
...
let rec string_of_stmt = function
  ...
  | If (cond, then_branch, Some else_branch) ->
      "if " ^ string_of_expr cond ^ " " ^ string_of_stmt then_branch ^
      " else " ^ string_of_stmt else_branch
  | While (cond, body) ->
...
let string_of_func_def func =
  ...
  let ret_type_str = match func.return_type with
    | Some typ -> ": " ^ string_of_type typ
    | None -> ""
  in
  "fn " ^ func.name ^ "(" ^ String.concat ", " param_strs ^ ")" ^
